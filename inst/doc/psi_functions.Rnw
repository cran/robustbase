...
all $x \ge x_r$ for $x_r < \infty$.  Corresponding to a redescending
...
\citet{MarRMY06} and has been called $\chi$ function in other contexts.
...
For the MM-estimators and their generalizations available via \texttt{lmrob()},
...
The constant $k$ for $95\%$ efficiency of the regression estimator is $0.9016$ and
the one for a breakdown point of $0.5$ of the S-estimator is $0.212$.
