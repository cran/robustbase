...
Martin: The above/below is \textbf{not} true : \texttt{"Welsh"} does \emph{not} have a finite
rejection point, but \emph{does} have bounded $\rho$, and hence well defined
$\rho(\infty)$, and we \emph{can} use it in \texttt{lmrob()}.
\\ $\to$ sent E-mail Oct.~18, 2014 to Manuel and Werner, proposing to
change the definition of ``redescending''.

\paragraph{Weakly redescending $\psi$ functions}.\
Note that this definition does require a finite rejection point $x_r$. Consequently,
...
generalization of the Welsh $\psi$-function (below). In \citet{ks2011} it is
defined as,
\begin{equation*}
  %% \label{eq:ggw}
  \psi_{a, b, c}(x) = \left\{
...
That approximation also avoids an anomaly for small
