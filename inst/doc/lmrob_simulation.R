...
###################################################
### code chunk number 2: graphics-setup (eval = FALSE)
###################################################
...
##                    `MMqT` = expression(paste('MM',q[T])),
##                    `MMqT.Wssc` = expression(paste('MM',q[T],'.Wssc')),
##                    `MMqE` = expression(paste('MM',q[E])),
##                    `MMqE.Wssc` = expression(paste('MM',q[E],'.Wssc')),
...
##                    `sigma_OLS` = expression(hat(sigma)[OLS]),
##                    `t1` = expression(t[1]),
##                    `t3` = expression(t[3]),
##                    `t5` = expression(t[5]),
##                    `cskt(Inf,2)` = expression(paste('cskt(',infinity,',2)'))
##                    )
...
lst <- unique(lst)
lst <- lst[sapply(lst, function(x) !is.null(x[[1]]))]
## convert to table
...
###################################################
### code chunk number 8: simulation-run (eval = FALSE)
###################################################
...
##           ## A
##           lsp2 <- rowSums(Mpsi(lresid,lctrl$tuning.psi,
##                                                     lctrl$psi)^2)
##           ## B
##           lspp <- rowSums(lpp <- Mpsi(lresid,lctrl$tuning.psi,
##                                                            lctrl$psi,1))
...
test.fixed <- droplevels(subset(test.1, n == 20))
test.1 <- droplevels(subset(test.1, n != 20))
test.lm <- droplevels(subset(test.1, Function == 'lm'))
test.lm$Psi <- NULL
test.lm.2 <- droplevels(subset(test.lm, Error == 'N(0,1)'))
test.1 <- droplevels(subset(test.1, Function != 'lm'))
test.2 <- droplevels(subset(test.1, Error == 'N(0,1)' & Function != 'lm'))
test.3 <- droplevels(subset(test.2, Method != 'SMDM'))
test.4 <- droplevels(subset(test.1, Method != 'SMDM'))
...
##       ylab(expression(sd(log(hat(sigma)))*sqrt(n))) +
##       facet_wrap(~ Psi) +
##       geom_point(data=test.lm, alpha=alpha.n, aes(color = Est.Scale)) +
...
##       scale_colour_discrete("Scale Est.", labels=lab(test.4$Est.Scale,
##                                           test.lm$Est.Scale)))
...
##                     labels=lab(tmp$Error)) +
##       facet_wrap(~ Psi) +
##       ylab(expression(paste('efficiency of ',hat(beta)))) +
##       scale_colour_discrete(name = "Estimator",
##                             labels = lab(tmp$Estimator)))
...
##       g.scale_y_log10_1() +
##       scale_shape_discrete(expression(n)) +
##       scale_colour_discrete(name = "Estimator", labels = lab(test.2$Estimator))  +
...
##       g.scale_y_log10_0.05() +
##       scale_colour_discrete(name = "Estimator", labels=lab(test.2$method.cov))  +
##       ylab(expression(paste("empirical level (", H[0], ": ", beta[1], "=", 0, ")"))) +
##       facet_wrap(~ Psi))
...
##       geom_point(aes(shape = Error), alpha = alpha.error) +
##       stat_summary(aes(x=ratio), fun.y=median, geom='line') +
##       geom_point(data=subset(test.lm, n == 25), alpha = alpha.n) +
##       stat_summary(data=tmp2 <- subset(test.lm, n == 25), aes(x=ratio),
##                    fun.y=median, geom='line') +
##       g.scale_shape("Error", labels=lab(tmp$Error)) +
##       scale_colour_discrete(name = "Estimator (Cov. Est.)"
##                             ,labels=lab(tmp$method.cov, tmp2$method.cov)
##                             ) +
##       ylab(expression(paste("empirical power (", H[0], ": ",
##           beta[1], "=", 0.2, ")"))) +
##       facet_wrap(~ Psi)
##       )
...
##       geom_point(aes(shape = Error), alpha = alpha.error) +
##       stat_summary(aes(x=ratio), fun.y=median, geom='line') +
##       geom_point(data=tmp2 <- subset(test.lm, n == 25), alpha = alpha.n) +
##       stat_summary(data=subset(test.lm, n == 25), aes(x=ratio),
##                    fun.y=median, geom='line') +
##       g.scale_shape("Error", labels=lab(tmp$Error)) +
##       scale_colour_discrete(name = "Estimator (Cov. Est.)"
##                             ,labels=lab(tmp$method.cov, tmp2$method.cov)
